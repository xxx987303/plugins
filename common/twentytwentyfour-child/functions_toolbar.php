 ...
 */
function YB_remove_toolbar_items() {
    global $wp_admin_bar;
...
add_action('wp_before_admin_bar_render', 'YB_remove_toolbar_items', 999);

/**
 * ADD A CUSTOM ROOT ITEM
 */
function YB_add_toolbar_root_item() {
    global $wp_admin_bar;
...
//add_action('admin_bar_menu', 'YB_add_toolbar_root_item', 16);

/**
 * ADD A SUB-ITEM TO AN EXISTING MENU
 */
function YB_add_toolbar_sub_item() {
    global $wp_admin_bar;
...
//add_action('admin_bar_menu', 'YB_add_toolbar_sub_item', 999);

/**
 * Add item with title and image
 */
function YB_toolbar_with_image( $wp_admin_bar ) {
    ...
    $html_title = "<img src='$image_url' width=20px height=20px style='vertical-align:middle; margin-right:8px; border-radius:50%;'/>Sweet Home\n";
...
add_action( 'admin_bar_menu', 'YB_toolbar_with_image', 9 );
